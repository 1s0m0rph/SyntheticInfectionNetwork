...
class Disease:
	...
	def disease_state_transition(self,person:ps.Person):
		...
		elif (person.disease_state[self] == 'ID') and (self.recover()):
			person.disease_state[self] = 'R'
		elif (person.disease_state[self] == 'ID') and (self.die(person)):
	...
	def infects(self,a : ps.Person,b : ps.Person):
		if (a.disease_state[self] != 'II') and (a.disease_state[self] != 'IS'):#all diseases should be accounted for in every person's disease_state map, so we don't need to check
			#a is not infectious
			return False
		...
		symptom_effect = self.symptom_infectivity_modifier if (a.disease_state[self] == 'IS') else 0.
		hand_wash_effect = 0.
		...
		infectivity = self.infectivity[a.currentActivity.activity_type] + symptom_effect + hand_wash_effect + intimate_effect + net_symbio_effect
